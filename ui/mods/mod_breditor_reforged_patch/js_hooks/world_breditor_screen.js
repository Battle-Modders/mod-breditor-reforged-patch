// Overwrite the relevant functions to make it compatible with Reforged
// Generally the original function is copy pasted and some stuff is modified to adapt to Reforged

WorldBreditorScreen.prototype.ModBreditorReforgedPatchCategories = [];

WorldBreditorScreen.prototype.FirstPerkButton = function (_moveforward)
{	
	var pt_classnames = this.ModBreditorReforgedPatchCategories;
	if (_moveforward)
	{
	...
	var tempo = this.mPTree[pt_classnames[this.mPerkButtons[0].Counter]][this.mPerkButtons[1].Counter].Name;
...
WorldBreditorScreen.prototype.loadFromData = function (_data)
{
	...
	this.ModBreditorReforgedPatchCategories = _data.ModBreditorReforgedPatchCategories;
...
/* 	if('SubTitle' in _data && _data.SubTitle !== null)
	...
	var pt_classnames = _data.ModBreditorReforgedPatchCategories;
	this.mPerkButtons[0].Button.changeButtonText(pt_classnames[this.mPerkButtons[0].Counter]);
	var tempo = this.mPTree[pt_classnames[this.mPerkButtons[0].Counter]][this.mPerkButtons[1].Counter].Name;
	this.mPerkButtons[1].Button.changeButtonText(tempo.replace('Tree','').replace('Class',''));
